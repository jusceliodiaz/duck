(function () {
  ...
  function avisar(texto, erro) {
    ...
    if (erro) message.setAttribute('data-erro', '1');
    else message.removeAttribute('data-erro');
  }
  ...
  form.addEventListener('submit', async function (e) {
    ...
    if (!email.checkValidity()) {
      ...
      avisar('Digite um e-mail válido para entrar na lista.', true);
      email.focus();
      return;
    }
    email.removeAttribute('aria-invalid');
    ...
    try {
      ...
      avisar('Você está na lista! Avisaremos quando a loja abrir.');
      form.reset();
    } catch (err) {
      avisar('A lista ainda não está disponível. Seu e-mail não foi cadastrado. Tente novamente mais tarde.', true);
    } finally {
